	...
	<div class="right_panel">
            ...
            <div class="dtls">
                <?php foreach($resume_details[0]['trainings'] as $tran){
                    ...
                    echo '<a href="'.$url.'resume/edit_train/'.$res_id.'/'.$tran['id'].'">Edit</a> | <a href="'.$url.'resume/delete_train/'.$res_id.'/'.$tran['id'].'">Delete</a>';
                }
?>
            </div>
            <?php } ?>
            ...
            <div id="create">
            <?php echo form_open('resume/edit_train/'.$res_id.'/'.$id);?>
            <h2>Edit Trainings & Courses</h2>
            ...
            echo validation_errors();?></div>
             ...
             <h3>Courses/Trainings/Workshops</h3>
                ...
                <div class="field"><input type="hidden" value="0" id="indexi"/><input type="text" class="inpt_fld" name="course_nam" id="course_nam" value="<?php if(!empty($trainings)){ echo $trainings[0]['course_name'];}elseif($this->input->post('course_nam') != ""){ echo $this->input->post('course_nam'); }?>"/></div>
                <div class="clearfix"></div>
                
                <div class="text">Conducted By:</div>
                <div class="field"><input type="hidden" value="<?php echo $id;?>" name="id"/><input type="text" class="inpt_fld" name="con_by" id="con_by" value="<?php if(!empty($trainings)){ echo $trainings[0]['conducted_by'];}elseif($this->input->post('con_by') != ""){ echo $this->input->post('con_by'); }?>"/></div>
                <div class="clearfix"></div>
                
                
                <div class="text">Location:</div>
                <div class="field"><input type="hidden" value="<?php echo $res_id;?>" name="res_id"/><input type="text" class="inpt_fld" name="location" id="location" value="<?php if(!empty($trainings)){ echo $trainings[0]['location'];}elseif($this->input->post('location') != ""){ echo $this->input->post('location'); }?>"/></div>
                <div class="clearfix"></div>
                </div>
                
                <div class="text">Date:</div>
                <div class="field"><input type="text" class="inpt_fld" name="con_date" id="con_date" value="<?php if(!empty($trainings)){ echo $trainings[0]['date'];}elseif($this->input->post('con_date') != ""){ echo $this->input->post('con_date'); }?>"/></div>
                <div class="clearfix"></div>
