...
function From( $from )
{
	...
	if( ! is_string($from) ) {
		...
		$msg=urlencode("Class Mail: error, From is not a string");
...
function contenttype( $ctype )
{
	...
	$this->xheaders["Content-Type"] = $ctype;
...
function CheckAdresses( $aad )
{
	...
	for($i=0;$i< count( $aad); $i++ ) {
		if( ! $this->ValidEmail( $aad[$i]) ) {
			...
			$msg = urlencode("Mail Method: Invalid Email Address");	
			header("Location: thankyou.php?msg=$msg");
			exit;
		}
	}
...
function _build_attachement()
{
	...
	for( $i=0; $i < count( $this->aattach); $i++ ) {
		...
		if( ! file_exists( $filename) ) {
			...
			$msg = urlencode("Class Mail, method attach : file $filename can't be found");
			header("Location: thankyou.php?msg=$msg");
			exit;
		}
