<?php 
// image media add
if($_POST['mode']=='add' && $_POST['act']=='manage_car_images' && $_POST['request_page']=='manage_caragency_images'){
$post=$_POST;
$error='';
...
if($_POST['mode']=='update' && $_POST['act']=='manage_car_images' && $_POST['request_page']=='manage_caragency_images'){
...
if($error!='')
{
			...
			header("Location: admin.php?errmsg=$msg&act=".$post['act2']."&id=".base64_encode($id));
			exit;
}
else
{
...
if($_GET['mode']=='delete' && $_GET['act']=='manage_car_images' && $_GET['request_page']=='manage_caragency_images'){
	$id=base64_decode($_GET['id']);
	
		$sel_qry = "SELECT fileurl FROM ".$tblprefix."agncy_mediafiles WHERE mf_id =".$id; 
		$rs_select = $db->Execute($sel_qry);
		
		$image_name=$rs_select->fields['agncy_mediafiles'];
