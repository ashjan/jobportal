<?php
$post=implode(",",$_POST);
$values=base64_encode($post);
//echo $values=base64_encode($_POST);
...
if($_POST['mode']=='add' && $_POST['act']=='manage_property_policy' && $_POST['request_page']=='policy_management'){
	...
	$_SESSION['internet_cost'] =$_POST['internet_cost'];
...
/*
	if($check_in_from1 < $check_out_from1){
		//$errmsg =base64_encode('Check-out-from must be less than check-in-from');
		$errmsg =base64_encode('Vrijeme odjvaljivanja mora bitti manje od vremena prijavljivanja');
	}*/
...
unset($_SESSION['add_sees']);
	if($rs_policy){
		...
		header("Location: admin.php?okmsg=$okmsg&act=manage_property_policy");
		exit;
	}else{
...
######################
//---------Edit Category---------
if($_POST['mode']=='update' && $_POST['act']=='manage_property_policy' && $_POST['request_page']=='policy_management'){
	...
	$internet_cost = addslashes(trim($_POST['internet_cost']));
	$_SESSION['internet_cost'] = trim($_POST['internet_cost']);
	...
	$parking_costs = addslashes(trim($_POST['parking_costs']));
	$_SESSION['parking_costs'] = trim($_POST['parking_costs']);
	$pets_allowed = addslashes(trim($_POST['pets_allowed']));
	$important_notice = addslashes(trim($_POST['important_notice']));
	...
	if($maximum_extra_beds !=0){
		if($children_age_beds == '' or !is_numeric($children_age_beds)){
			...
			header("Location: admin.php?act=editpolicy&errmsg=$errmsg&id=".$_POST['id']);
			exit;
			if($extra_bed_charges == '' or !is_numeric($extra_bed_charges)){
				//$errmsg = base64_encode('Please Enter Extra Bed Charges with Integer Value');
				$errmsg = base64_encode('Cijena dodatnih kreveta mora biti numerička');
		...
		if($property_name == ''){
			//$errmsg = base64_encode('Please Enter Property Name');
			$errmsg = base64_encode('Molimo unesite naziv objekta');
			header("Location:admin.php?act=editpolicy&errmsg=".$errmsg."&id=".base64_encode($id));
			exit;
		}
	...
	// insert for montenegro language for offline food beverage 
	//$last_id = mysql_insert_id();
	$property_name_qry = "SELECT  count( * ) AS totallang FROM `tbl_language_contents` WHERE  
...
//--- insert for montenegro language for offline Important Notices
...
// insert for Russian language for offline Important Notices
	//$last_id = mysql_insert_id();
	$property_name_qry = "SELECT  count( * ) AS totallang FROM `tbl_language_contents` WHERE  
...
if($_GET['mode']=='del_policy' && $_GET['act']=='manage_property_policy' && $_GET['request_page']=='policy_management'){
