...
$qry_faq = "SELECT * FROM ".$tblprefix."testimonials testim  ORDER BY testim.id ASC " ;
...
$qry_limit = "SELECT 
              ...
              tbl_testimonials.testim_orderdering,
              usr.first_name,
              usr.last_name,
              usr.profile_pic
              FROM 
...
<table width="100%" border="0" class="table table-hover" cellspacing="2" cellpadding="2" class="table">
	
    <tr>
    	<td colspan="8" align="center" <?php if(!empty($_GET['okmsg'])){ echo 'class="okmsg"'; }else{ echo 'class="errmsg"';} ?> ><?php if(!empty($_GET['errmsg'])){ echo base64_decode($_GET['errmsg']).base64_decode($_GET['okmsg']);}?></td>
    </tr>
 	<tr class="tabheading">
    	<td colspan="5" align="right">Total Number Of testimonials Found: <?php echo $totalcountalpha ?></td>
	</tr>
<!--	<tr class="tabheading">
 ...
 <div id="controls" class="add_subscriber">
  <table class="table table-hover" cellpadding="1"  cellspacing="1" border="0" width="100%" >
  <tr>
  <td colspan="2">
...
<table class="table table-hover" cellpadding="1" cellspacing="1" border="0" >				
<!--				<tr>
...
<!--		<input type="hidden" name="id" value="<?phpecho  echo base_en64code($id); ?>">-->
 ...
 <form name="ordering_menu" action="admin.php" method="post" onsubmit="return validate_contant()" enctype="multipart/form-data"> 
	<td>
		<table class="table table-hover" width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="table">
		    <tr height="5%">
			  <td colspan="8" ></td>
		    </tr>
			<tr class="tabheading">
...
<!--				<th width="5%">Options</th>-->
 ...
 <input size="3" style="text-align:center; width:50px; background:#FFFFDD; border:#0000FF 1px solid;" name="menu_order[<?php echo $rs_limit->fields['id']; ?>]" id="menu_order[<?php echo $rs_limit->fields['id']; ?>]" type="text" value="0"/>
 					<?php }else{ ?>
					<input size="3" style="text-align:center; width:50px; background:#FFFFDD; border:#0000FF 1px solid;" name="menu_order[<?php echo $rs_limit->fields['id']; ?>];" id="menu_order[<?php echo $rs_limit->fields['id']; ?>]" type="text" value="<?php echo $rs_limit->fields['testim_orderdering'];?>"/>
				<?php	} ?>
					</td>
                        <td valign="top"><?php echo $rs_limit->fields['testimonial']; ?></td>
                        <td><?php echo $rs_limit->fields['first_name'].' '.$rs_limit->fields['last_name']; ?></td>
                        <td>
                        
                            <?php if($rs_limit->fields['status']==0){ 
					  ?>
					  <a href="admin.php?act=testimonials_listing&amp;m_status=0&amp;mode=change_pmstatus&amp;id=<?php echo base64_encode($rs_limit->fields['id']); ?>&amp;request_page=testimonials_manager">
					  <img src="<?php MYSURL?>graphics/deactivate.gif" title="Make Default" border="0" />
					  </a>
					  <?php }else{ 
					   ?>
                        <a href="admin.php?act=testimonials_listing&amp;m_status=1&amp;mode=change_pmstatus&amp;id=<?php echo base64_encode($rs_limit->fields['id']); ?>&amp;request_page=testimonials_manager">
