...
$qry_faq = "SELECT mi.*,pm.id as pid,pm.first_name,pr.id as prid,pr.property_name  FROM `tbl_mediaimages` as mi INNER JOIN tbl_properties as pr ON pr.id=mi.property_id INNER JOIN tbl_users as pm ON pm.id=mi.pm_id";
...
$qry_limit = "SELECT mi.*,pm.id as pid,pm.first_name,pr.id as prid,pr.property_name  FROM `tbl_mediaimages` as mi INNER JOIN tbl_properties as pr ON pr.id=mi.property_id INNER JOIN tbl_users as pm ON pm.id=mi.pm_id";
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="txt">
	<tr>
    	<td id="heading">Manage Yatch Inventory Images</td>
  	</tr>
    <tr>
    	<td colspan="8" align="center" <?php if(!empty($_GET['okmsg'])){ echo 'class="okmsg"'; }else{ echo 'class="errmsg"';} ?> ><?php if(!empty($_GET['errmsg'])){ echo base64_decode($_GET['errmsg']).base64_decode($_GET['okmsg']);}?></td>
    </tr>
 	<tr class="tabheading">
    	<td colspan="5" align="right">Total Yatch Inventory Images Found: <?php echo $totalcountalpha ?></td>
	</tr>
	<tr class="tabheading">
...
<!--		<a   href="<?php //MYSURL?>admin.php?act=add_categories"   href="javascript:;" onClick="jQuery('#controls').slideToggle('fast'); return false"  ><img src="<?php //MYSURL?>graphics/add.png" border="0" title="Add Country" /></a>-->
 ...
 <div id="controls" class="add_subscriber">
	<form name="managecontentfrm" action="admin.php" method="post" onsubmit="return validate_contant()" enctype="multipart/form-data">
		...
		<table>
		<tr>
					<td class="txt1">Yatch Inventory Manager Name</td>
					<td>
					<select name="first_name" class="fields" id="first_name" onchange="get_prop_name('property_name', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name.php"?>')">
				 	<option value="">Select Yatch Inventory Manager</option>
				...
				<tr>
					<td class="txt1">Yatch Inventory Name</td>
					<td>
					<div id="property_name">
						<select name="property_name" class="fields" id="property_name">
							<option value="0">First Select Yatch Inventory Manager</option>
						</select>
					</div>
		...
		<input type="hidden" name="act" value="yatchinventoryimages" />
		 <input type="hidden" name="theValue" id="theValue" value="0" />
		<input type="hidden" name="request_page" value="manageyatchinventoryimages" />
  ...
  <tr>
    <td>
		<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="txt">
			...
			<tr class="tabheading">
				<td width="5%">Sr#</td>
				<td width="20">Yatch Inventory Manager</td>
			...
			?>
					<tr <?php if($i%2==0){ ?> bgcolor="#E7DAE7"  <?php }else{ echo 'bgcolor="#FFFFFF"'; }?>>
					  ...
					  ?></td>
						...
						<td valign="middle">
							<a href="admin.php?act=edityatchinventoryimages&amp;id=<?php echo base64_encode($rs_limit->fields['id']);?>"><img src="<?php MYSURL?>graphics/edit.gif" border="0"  title="Edit" /></a>&nbsp;&nbsp;	
							<a href="admin.php?act=yatchinventoryimages&amp;mode=delete&amp;id=<?php echo base64_encode($rs_limit->fields['id']); ?>&amp;request_page=manageyatchinventoryimages" onClick="return confirm('Are you sure you want to Delete?')"><img src="<?php MYSURL?>graphics/delete.gif" title="Delete" border="0" /></a>
                       </td>
					</tr>
			<?php 
						$rs_limit->MoveNext();
			...
			?>
					...
					<tr>
						<td colspan="2">
							<input type="hidden" name="act" value="manageyatchinventoryimages">		
			...
			?>
				<tr>
					<td colspan="14" class="errmsg"> No Yatch Inventory Image Found</td>
				</tr>
			<?php
				}// end if($totalcount > 0)
			?>
