...
if($_SESSION[SESSNAME]['pm_moduleid']==2){
	$module_pm_where = ' AND pr.pm_id = '.$_SESSION[SESSNAME]['pm_id'].' AND pr.pm_type=0  AND pr.id='.$id;
}else{
    $pm_id=base64_decode($_GET['pm_id']);
	$module_pm_where = ' AND pr.pm_type=0  AND  pr.pm_id ='.$pm_id.'  AND pr.id='.$id;
}
...
if($_SESSION[SESSNAME]['pm_moduleid']==2){
	...
	$qry_pm = "SELECT ".$tblprefix."property_manager.*,
				      ...
				      ".$tblprefix."properties.property_category 
						 ...
						 WHERE ".$tblprefix."properties.pm_type=0  AND  ".$tblprefix."property_manager.id = ".$pm_id."   
...
$qry_sub_type = "SELECT * FROM ".$tblprefix."business_subtype WHERE business_type_id=".$rs_limit->fields['business_type']; 
//   $qry_sub_type = "SELECT * FROM ".$tblprefix."business_subtype WHERE ".$tblprefix."business_subtype.business_category_id=24";
...
<td colspan="2" style="font-weight:bold; text-align:left; border:1px solid #999999; border-bottom:none;">Property Owner</td>
		...
		<tr>
			...
			<select name="first_name" class="fields"   id="first_name" >
				...
				?>
				><?php echo $rs_pm->fields['first_name']." ".$rs_pm->fields['last_name'] ;?>								 	  				</option><?php
					$rs_pm->MoveNext();
				}
		?>					
		    </select>				
