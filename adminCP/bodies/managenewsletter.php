...
if(isset($_POST['send_mail'])){
 ...
 if($group_selected==1){
	...
	if($group_name=='all_subscribers'){
		...
		$qry_faq = "SELECT email_address FROM ".$tblprefix."users WHERE pm_status=1 ORDER BY first_name ASC" ;
		$rs_faq = $db->Execute($qry_faq);
		$count_add =  $rs_faq->RecordCount();
		$totalRows = $count_add;
		$totalPages = ceil($totalRows/$maxRows);
		$qry_limit = "SELECT email_address FROM ".$tblprefix."users WHERE pm_status=1 ORDER BY first_name ASC LIMIT $startRow,$maxRows"; 
		$rs_limit = $db->Execute($qry_limit);
		$totalcountalpha =  $rs_limit->RecordCount();
		$i=1 ;
		$qrysub=mysql_query("SELECT email_address FROM ".$tblprefix."users WHERE pm_status=1 ORDER BY first_name ASC");
		while($res_sub=mysql_fetch_array($qrysub)){
    ...
    <td>
		<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="txt">
			...
			//	if($group_name=='all_subscribers'){
				...
				$rs_newsletter = $db->Execute($newsletter_qry);
			//	}elseif($group_name=='existing_pms'){
			//	$newsletter_qry = "SELECT email_address FROM ".$tblprefix."tbl_users WHERE pm_status=1 ORDER BY first_name ASC";
			//	$rs_newsletter = $db->Execute($newsletter_qry);
			//	}elseif($group_name=='top_pms'){
