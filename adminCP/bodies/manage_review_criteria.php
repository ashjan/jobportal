...
$qry_faq = "SELECT * FROM ".$tblprefix."review_criteria  ORDER BY ".$tblprefix."review_criteria.criteria_ordering ASC " ;
$rs_faq = $db->Execute($qry_faq);
$count_add =  $rs_faq->RecordCount();
$totalRows = $count_add;
$totalPages = ceil($totalRows/$maxRows);

$qry_limit = "SELECT * FROM ".$tblprefix."review_criteria ORDER BY ".$tblprefix."review_criteria.criteria_ordering ASC LIMIT $startRow,$maxRows"; 
...
<div class="border_div_categories"  align="center">				
		...
		<input type="hidden" name="act" value="manage_review_criteria">
		<input type="hidden" name="act2" value="manage_review_criteria">
<!--		<input type="hidden" name="id" value="<?phpecho  //echo base_en64code($id); ?>">-->
		<input type="hidden" name="request_page" value="review_criteria_management" />
		</td>
		</tr>
		</table>
</div>
...
<!--		<input type="hidden" name="mode" value="add">
 ...
 <input size="3" style="text-align:center; width:50px; background:#FFFFDD; border:#0000FF 1px solid;" name="menu_order[<?php echo $rs_limit->fields['id']; ?>]" id="menu_order[<?php echo $rs_limit->fields['id']; ?>]" type="text" value="0"/>
				...
				<?php	} ?>
						...
						<td valign="top">
							<a href="admin.php?act=update_review_criteria&amp;id=<?php echo base64_encode($rs_limit->fields['id']);?>"><img src="<?php MYSURL?>graphics/edit.gif" border="0" title="Edit" /></a>&nbsp;&nbsp;
							<a href="admin.php?act=manage_review_criteria&amp;mode=del_review_criteria&amp;id=<?php echo base64_encode($rs_limit->fields['id']); ?>&amp;request_page=review_criteria_management" onClick="return confirm('Are you sure you want to Delete?')"><img src="<?php MYSURL?>graphics/delete.gif" title="Delete" border="0" /></a>
                       </td>
					</tr>
			<?php 
						$rs_limit->MoveNext();
