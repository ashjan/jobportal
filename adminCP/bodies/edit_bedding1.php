...
//List down all Properties
 if($_SESSION[SESSNAME]['pm_moduleid']==2){
 	$qry_properties = "SELECT * FROM ".$tblprefix."properties WHERE pm_id=".$_SESSION[SESSNAME]['pm_id']." 
						AND pm_type=0
...
<?php	}else{ ?>
    <tr>
			...
			<td>
	<select name="pm_id" class="fields"   id="pm_id" onchange="get_edit_prop_nam_0('property_id', this.value, '<?php echo MYSURL."ajaxresponse/get_edit_prop_nam_0.php"?>')">
...
<?php	}else{ ?>  
		  <tr>
			...
			<td>
		<div id="property_id"> 	
		<select name="property_id" class="fields"   id="property_id" onchange="get_edit_prop_nam_0('room_id', this.value, '<?php echo MYSURL."ajaxresponse/get_edit_prop_nam_0.php"?>')">
			<option value="0">Izaberite objekat</option>
		<?php while(!$rs_properties->EOF){?>
       <option value="<?php echo $rs_properties->fields['id']; ?>" 
       <?php if($rs_properties->fields['id']==$rs_limit->fields['property_id']){ echo 'selected="selected"';}?>>
   ...
   <input type="hidden" name="rus_lan_id" id="rus_lan_id"     value="<?php echo $rus_lan_id; ?>"   />
		...
		<td>&nbsp;</td><td>&nbsp;<br/><br/></td>
		   ...
		   <?php echo $rs_limit->fields['dimensions_length']*0.4."&nbsp;in";  ?>
			...
			<input type="hidden" name="act2" value="manage_bedding1" />
