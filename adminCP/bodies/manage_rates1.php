...
<?php	}else{ ?>
          <tr>
             <td>PM Name</td>
              <td>
			    <select name="first_name" id="first_name" class="fields" onchange="get_prop_name_rate('property_id1', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name_rate.php"?>')">
          ...
          <td>
		   <div id="property_id1"> 
			    <select name="property_id" id="property_id" class="fields" onchange="get_rooms1('rooms_id1', this.value,<?php echo $_SESSION[SESSNAME]['pm_id'];?>, '<?php echo MYSURL."ajaxresponse/get_rooms1.php"?>')">
...
<?php }else{ ?>
	...
	<div id="rooms_id1">
	    ...
	    <tr bgcolor="#CCCCCC">
         ...
         <br/>
         <span style="color:#FF0000;"><strong>Warning:</strong> Replacing the date ranges will delete all the previous date ranges for this rate.
		  </span>
          <br/>
      ...
      <tr>
		<td colspan="3" >	
			...
			<!-- <tr>
			 <td bgcolor="#3399FF">Advance User Option</td>
			 <td>
			 <select name="advance_use_option" class="fields"  id="advance_use_option" 
			 onchange="jQuery('#advance_rate_option').toggle('slow');">
				<option value="1"  name="single_use_option">YES</option>
				<option value="0"  name="single_use_option">NO</option>
			</select>
			</td>
			 <td>&nbsp;</td>
			 </tr>-->
	...
	<tr>
	   <td colspan="2">
		<tr>
			 <td bgcolor="#3399FF" style="width:198px;">Advance Single User Option</td>
			 <td><input type="radio" name="single_adv_use_option" id="single_adv_use_option1" onClick="jQuery('#advance_single_use').show('slow');" checked="checked"  value="1"/>YES  &nbsp; &nbsp;
				<input type="radio" onClick="jQuery('#advance_single_use').hide('slow');" name="single_adv_use_option" id="single_adv_use_option0"  value="0"/>NO
			</td>
		</tr>
		<tr>
		<td colspan="2">	
    ...
    <td colspan="3">
     <table width="100%" border="0" cellspacing="1" cellpadding="2" class="txt" >
		<tr>
	      ...
	      <input type="hidden" name="act" value="manage_rates1" />
...
<table cellpadding="1" cellspacing="2" border="0">
	...
	<tr>				
					...
					<td>
					<select name="pm_id" class="fields" id="pm_id" onchange="get_prop_name25('property_id2', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name25.php"?>')">
				 	<option value="0">Izaberite vlasnika objekta :</option>
					<?php
					if(isset($_GET['pm_id'])){
					...
					if(isset($_GET['pr_id'])){
						...
						if(isset($_GET['pr_id']) and isset($_GET['pm_id']) and !empty($_GET['pr_id']) and !empty($_GET['pm_id'])){
                    	 $property_qry = "SELECT id,property_name FROM ".$tblprefix."properties WHERE pm_id=".base64_decode($_GET['pm_id']). "	 					
											AND ".$tblprefix."properties.pm_type=0 
											AND ".$tblprefix."properties.property_category=24";
					...
					if(isset($_GET['pr_id'])){
						...
						//Dropdown for parent
						if(isset($_GET['pr_id']) and isset($_GET['pm_id']) and !empty($_GET['pr_id']) and !empty($_GET['pm_id'])){
                    	 $property_qry = "SELECT id,room_type FROM ".$tblprefix."rooms WHERE pm_id=".base64_decode($_GET['pm_id']). " 
						 					AND ".$tblprefix."properties.pm_type=0 
											AND ".$tblprefix."properties.property_category=24";
						$rs_prop = $db->Execute($property_qry);
						$totalprope =  $rs_prop->RecordCount();
                    ...
                    <td>
                    <div id="property_id">
					<select name="property_id"class="fields" onchange="get_rooms1('rooms_id2', this.value,<?php echo $_SESSION[SESSNAME]['pm_id']; ?> , '<?php echo MYSURL."ajaxresponse/get_rooms1.php"?>')">
				 	<option value="0">Izaberite objekat</option>
					<?php  
					$rs_property->MoveFirst();
					while(!$rs_property->EOF){ ?>
 <option value="<?php echo $rs_property->fields['id']; ?>" ><?php echo $rs_property->fields['property_name']; ?></option>
					...
					<select name="room_type"class="fields" onchange="get_rates1('get_rates1', this.value, '<?php echo MYSURL."ajaxresponse/get_rates1.php"?>')" >
				 	<option value="0">Izaberite sobu</option>
					<?php  
					/*
					$rs_region->MoveFirst();
