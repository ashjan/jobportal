<?php
	...
	if($_SESSION[SESSNAME]['pm_moduleid']==2){
		$module_pm_where = ' WHERE pp.pm_id = '.$_SESSION[SESSNAME]['pm_id'] ." AND pr.pm_type=1 AND pr.property_category=24";
	}else{
...
<table cellpadding="1" cellspacing="1" border="0" >
		...
		?>	
				...
				<select name="property_name" class="fields"   id="property_name">
						...
						<?php 
						$qry_prop = "SELECT id,property_name FROM ".$tblprefix."properties WHERE property_category=24 AND pm_id=".$_SESSION[SESSNAME]['pm_id'].' AND pm_type=1';
				...
				<tr>
					...
					<td>               
						...
						$qry_prop = "SELECT id,property_name FROM ".$tblprefix."properties WHERE property_category=24 AND pm_type=1 AND pm_id=".$_SESSION['add_sees']['first_name'];
                        $rs_prop = $db->Execute($qry_prop);
						$count_prop =  $rs_prop->RecordCount();
						$totalprop = $count_prop;
						$rs_prop->MoveFirst();
...
value="<?php if (!empty($_SESSION['add_sees']['check_out_from'])){ echo $_SESSION['add_sees']['check_out_from']; }?>" />
		...
		<td class="txt" style="width:140px;">	Check Out Untill <br/>[Odjavljivanje do]</td>
		<td style="width:100px;">
		...
		<tr>
			<td colspan="2" class="txt">
				<table border="0" cellpadding="2" cellspacing="1" id="baby_costs" width="77%" <?php if(isset($_SESSION['add_sees']['children_age']) and $_SESSION['add_sees']['maximum_baby_cots']!=0){?> style="display:block;"<?php }else{?> style="display:none;"<?php }?>>
				<tr>
	    ...
	    <tr>
		<td colspan="2" class="txt">
				<table border="0" cellpadding="2" cellspacing="1" width="77%" id="extra_beds">
			       <tr>
				   <td style="width:228px;"class="txt" >Children Age For Free of Charge When Using Existing Beds <br/>
				   [Godine do kojih djeca borave besplatno ako koriste postojeći krevet(Godina)]
				   </td>
                   <td style="width:279px;"class="txt">
			...
			<table border="0" cellpadding="2" cellspacing="1" id="extra_bed_charges_table" <?php if(isset($_SESSION['add_sees']['extra_bed_charges']) and $_SESSION['add_sees']['maximum_extra_beds']!=0){?> style="display:block" <?php }else{?> style="display:none;" <?php }?>>
  			<tr>
			<td class="txt" style="width:246px;">Extra Bed Charges<br/>[Cijena dodatnog kreveta]</td>
			<td class="txt" >
<input type="text" name="extra_bed_charges" class="fields" id="extra_bed_charges" 
 value="<?php if(!empty($_SESSION['add_sees']['extra_bed_charges'])){ echo $_SESSION['add_sees']['extra_bed_charges'];} ?>" /> &nbsp; EUR
		...
		<tr>
		<td colspan="2"  style="text-align:left; font-weight:bold;" class="txt">Cancellation Policy<br/>[Otkazna politika]</td>
		</tr>
		<tr><td class="txt">Cacellation Days<br/>[Otkazni dani]</td>
			<td>
		...
		<tr><td class="txt">
			...
			value="1">Cijena jednog noćenja će biti naplaćena</option>
			<option 
			<?php if(!empty($_SESSION['add_sees']['no_show_policy'])){ 
				if($_SESSION['add_sees']['no_show_policy']==0){ echo 'selected="selected"';}
				} ?>
			value="0">Ukupna cijena noćenja će biti naplaćena</option>
			</select>
			</td></tr>		
		<tr>
		<td colspan="2"  style="text-align:left; font-weight:bold;" class="txt">&nbsp;</td>
...
<div class="border_div_categories"  align="center">				
		<table cellpadding="1" cellspacing="1" border="0" >
		<tr>
		<td><input style="margin:5px; width:206px; float:none; text-align:center;" name="addsubscribeSbt" id="addsubscribeSbt" type="submit" value="Add Policy &nbsp; [Dodaj politiku objekta]" class="button" />
   ...
   <tr>
    ...
    <td>       
        ...
        ?>
           ...
           <select name="pm_id" class="fields" id="pm_id" onchange="get_prop_property('property_id1', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_property.php"?>')">
    ...
    <td width="70%" align="center">
    <div id="property_id1">		
        <select name="property_id" class="fields" id="property_id" onChange="get_prop_property_policy('get_prop_property_policy',this.value,<?php echo $_SESSION[SESSNAME]['pm_id'];?>, '<?php echo MYSURL."ajaxresponse/get_prop_property_policy.php"?>')">
        <?php
        if($count_add<=0){?>
        <option value="0">Izberite objekat</option>
        <?php
