...
$qry_faq = "SELECT * FROM ".$tblprefix."property_category  ORDER BY ".$tblprefix."property_category.cat_orderdering ASC " ;
$rs_faq = $db->Execute($qry_faq);
$count_add =  $rs_faq->RecordCount();
$totalRows = $count_add;
$totalPages = ceil($totalRows/$maxRows);

$qry_limit = "SELECT * FROM ".$tblprefix."property_category ORDER BY ".$tblprefix."property_category.cat_orderdering ASC LIMIT $startRow,$maxRows"; 
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="table">
    ...
    <tr>
 	...
 	<tr class="tabheading">
    	<td colspan="5" align="right">Total Number Of Profile Categories Found: <?php echo $totalcountalpha ?></td>
	</tr>
	<tr class="tabheading">
...
<!--		<input type="hidden" name="id" value="<?phpecho  echo base_en64code($id); ?>">-->
	...
	<td>
		<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="table">
			...
			<tr class="tabheading">
                ...
                <th width="50%">Profile Category</th>
 ...
 <input size="3" style="text-align:center; width:50px; background:#FFFFDD; border:#0000FF 1px solid;" name="menu_order[<?php echo $rs_limit->fields['id']; ?>]" id="menu_order[<?php echo $rs_limit->fields['id']; ?>]" type="text" value="0"/>
				...
				<?php	} ?>
						...
						<td valign="top">
							<a href="admin.php?act=update_property&amp;id=<?php echo base64_encode($rs_limit->fields['id']);?>"><img src="<?php MYSURL?>graphics/edit.gif" border="0" title="Edit" /></a>&nbsp;&nbsp;
							<a href="admin.php?act=manage_property&amp;mode=del_category&amp;id=<?php echo base64_encode($rs_limit->fields['id']); ?>&amp;request_page=property_management" onClick="return confirm('Are you sure you want to Delete?')"><img src="<?php MYSURL?>graphics/delete.gif" title="Delete" border="0" /></a>
                       </td>
					</tr>
			<?php 
						$rs_limit->MoveNext();
