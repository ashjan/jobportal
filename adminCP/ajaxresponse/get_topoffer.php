...
//$qry_avgrating1 = "SELECT AVG(rating) as proprating,property_id,pm_id FROM ".$tblprefix."property_reviews GROUP BY property_id ORDER BY proprating DESC LIMIT 0,5";
					...
					$qry_avgrating1 ="SELECT
									...
									Left Join ".$tblprefix."admn_proprating ON ".$tblprefix."admn_proprating.proprty_id = ".$propid."
									left Join ".$tblprefix."property_reviews ON ".$tblprefix."property_reviews.property_id = ".$propid." WHERE ".$tblprefix."properties.permission_status=1 AND (".$tblprefix."properties.topoffr_flag=1 or ".$tblprefix."properties.goldoffr_flag=1) AND ".$tblprefix."properties.id=".$propid."  
									ORDER BY proprating DESC, rating_sum DESC LIMIT 0,5";
...
WHERE prop.permission_status=1 and (prop.topoffr_flag=1 or prop.goldoffr_flag=1) AND prop.id=".$propid."
		...
		if($pageNum==0)
			   ...
			   ?>
					   ...
					   <a href="admin.php?act=top_offersmngmnt&amp;mode=senmailtopm&amp;offerid=<?php echo base64_encode($rs_topoffr->fields['pgm_id']); ?>&amp;request_page=topoffrmang">Send Email</a>
						 ...
						 */?></td>-->			
						<td valign="top"><?php 
						if($rs_definvice->fields['invoic_charg_amnt']!=""){
							echo $rs_definvice->fields['invoic_charg_amnt'];
						}else{
