...
$property_id=$_GET['room_type_id'];//property id for example serena view
$pm_id = $_GET['pm_id'];
$rooms = $_GET['rooms'];
$propertyroomid = $_GET['property_id'];//rooms id for example 3 bed rooms
$day = $_GET['date'];
$date_con = date("m/d/Y",$day);

$qry_select = "SELECT * FROM ".$tblprefix."standard_date WHERE standard_date='".$date_con."' AND pm_id='".$pm_id."' AND property_id='".$property_id."' AND room_type_id='".$propertyroomid."'";
...
<input type="hidden" name="propertyroomid" id="propertyroomid" value="<?php echo $propertyroomid; ?>">
 <!--<div  style="display:block; position:absolute;display: block;float: left; position: absolute; width: -moz-max-content; width: -webkit-max-content; width: -o-max-content;"> -->
		...
		if(!empty($original_day)){
				...
				$inner_div='<div class="inner_div">
              	...
              	foreach($days as $day){
					   if(in_array($day,$first_day_changed)){
					    ...
					    switch ($remaining_rooms){
					    	...
					    	case (0):?>
					    	<div  class="value_div"><img src="graphics/red.jpg" title="<?php echo $day.",remaining rooms=".$remaining_rooms.",single rate price=".$rs_limit->fields['single_rate_price'].",standard rate price=".$rs_limit->fields['standard_rate_price']?>" border="0"/></div>
					    <?php 
					    		break;
					    	case ($remaining_rooms>0):?>
					    	
					    	<div  class="value_div"><img src="graphics/green.jpg" title="<?php echo $day.",remaining room=".$remaining_rooms.",single rate price=".$rs_limit->fields['single_rate_price'].",standard rate price=".$rs_limit->fields['standard_rate_price']?>" border="0"/></div>
					    <?php 
					    		break;
					...
					<?php    		
              	foreach($days as $$day){?>
					<?php echo '<div  class="value_div">'.$rs_limit->fields['single_rate_price'].'</div>'; ?>
					<?php }?>
                    ...
                    <div class="single_div overview_heading1">
     				<?php    		
              	foreach($days as $day){?>
					<?php echo  '<div  class="value_div">'.$rs_limit->fields['standard_rate_price'].'</div>';?>
              	...
              	foreach($days as $day){
					   ...
					   ?>	
					   <div  class="value_div" >
					   <a onclick="return open_text_box1('<?php echo $pm_id;?>','<?php echo $propertyroomid?>','<?php echo $find_room;?>','<?php echo strtotime($day);?>')"><?php echo $find_room;?></a>
					   </div>
					  <?php 
					   }else {?>
				<div  class="value_div">
				<a onclick="return open_text_box1('<?php echo $pm_id;?>','<?php echo $propertyroomid?>','<?php echo $rs_limit->fields['rooms_for_sale'];?>','<?php echo strtotime($day);?>')">
					   	<?php echo $rs_limit->fields['rooms_for_sale'];?></a></div>
					   <?php }
					 }?>
...
<?php
 ...
 ?>				<p class="errmsg">No rates found</p>
				...
				<div style="float:left; width:100%;">
				<table id="textbox1" style="display:none">
				<tr>
				<td><input type="text" name="value_of_rooms1" value="" id="value_of_rooms1" /></td>
				<td><a onclick="return set_property_room_value1('newroomvalues','<?php echo MYSURL."ajaxresponse/set_property_room_value.php"?>');">Change value</a></td>
				</tr>
				<input type="hidden" name="value_pmi_d1" id="value_pm_id1">
				<input type="hidden" name="value_room_id1" id="value_room_id1">
				<input type="hidden" name="value_day_id1" id="value_day_id1">
