...
if(isset($_GET['id'])){ 
		...
		$qry_content = "SELECT id, room_type FROM  ".$tblprefix."rooms 
						WHERE property_id=".$id."  AND pm_id=".$pm_id.""; 
		$rs_content = $db->Execute($qry_content);
		$count_add =  $rs_content->RecordCount();
		?>
<div id="rooms_id1">		
<select name="room_id" class="fields" id="sel_room_id" onChange="get_rates_room_manag('get_rates_videos', this.value,<?php echo $id;?>,<?php echo $pm_id;?> ,'<?php echo MYSURL."ajaxresponse/get_room_manag.php"?>')">
<?php
if($count_add<=0){?>
