...
//Add Section
if($_POST['mode']=='add' && $_POST['act']=='manage_rates' && $_POST['request_page']=='rates_management'){	
		...
		$strooms_for_sale         = addslashes(trim($_POST['rooms_for_sale']));
		...
		$advrooms_for_sale = addslashes(trim($_POST['advnc_rooms_for_sale']));
			...
			if($standard_start_date == ''){
				//$errmsg = base64_encode('Please Enter Standard Start Rates');
			...
			if($advance_use_option!=0){
				...
				$advance_end_date = '';
         					 ...
         					 $insert_tariff_cal = "INSERT INTO ".$tblprefix."standard_rates  
										...
										stndrd_rooms_for_sale = '".$rooms_for_sale."',
				...
				if(!empty($advance_ranges)){
					
					for($j=0;$j<count($advance_ranges);$j++){
						if(in_array($advance_ranges[$j],$check_advance_date_array)){
							
							$update_advance_query = "UPDATE ".$tblprefix."changed_advance_rates 
							                 SET advance_rate_price =".$advance_rate_price.",
							                 adv_rooms_for_sale =".$advrooms_for_sale."
							                 WHERE advance_date='".$advance_ranges[$j]."'
							                 AND room_type_id=".$room_type_id."
							                 AND property_id=".$property_name."
							                 AND pm_id=".$first_name."
							                 ";
				...
				*/
				...
				$okmsg = base64_encode("Osnovna cijena uspješno dodata !");
					header("Location: admin.php?okmsg=$okmsg&act=manage_rates");
					exit;
				}else{
...
//Update Section
if($_POST['mode']=='update' && $_POST['act']=='edit_rates' && $_POST['request_page']=='rates_management'){     
		...
		//validation start
			...
			if($standard_rate_price == ''){}elseif(!is_numeric($standard_rate_price)){
...
if($_GET['mode']=='delete' && $_GET['act']=='manage_rates' && $_GET['request_page']=='rates_management'){
	$id=base64_decode($_GET['id']);
		...
		$rs_delete = $db->Execute($del_qry);
		    ...
		    $qry_del_adv_single_date = "DELETE FROM ".$tblprefix."changed_advance_rates WHERE standard_rate_id='".$id."'";
