<?php	
if($_POST['mode']=='add' && $_POST['act']=='vat_and_tax_charges_management' && $_POST['request_page']=='manage_vat_and_tax_charges'){
	$post=$_POST;
	$error='';
	...
	}else{						if($post['vat_type_percent']== 1 && $post['vat_amount'] > 100){
...
if($_POST['mode']=='update' && $_POST['act']=='vat_and_tax_charges_management' && $_POST['request_page']=='manage_vat_and_tax_charges'){
	$id = base64_decode($_POST['id']);
	$post=$_POST;	
	$error='';
	if($post['property_name']==0){
   		//$errmsg=base64_encode("Please Select Property Name");
		$errmsg=base64_encode("Odaberite naziv vlasništva");
		header("Location: admin.php?errmsg=$errmsg&act=edit_vat_and_tax_charges&id=".base64_encode($id));
		exit;
	...
	if($error!=''){
		$msg=base64_encode($error);
			header("Location: admin.php?errmsg=$msg&act=edit_vat_and_tax_charges&id=".$_POST['id']);
	}else{
...
if($_GET['mode']=='delete' && $_GET['act']=='vat_and_tax_charges_management' && $_GET['request_page']=='manage_vat_and_tax_charges'){
