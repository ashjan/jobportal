<?php
if($_POST['mode']=='add' && $_POST['act']=='offers_not_included_in_price' && $_POST['request_page']=='ofrs_not_inc_in_price_mange')
		...
		$slug=slugcreation($event_category_name);
			...
			if($event_category_name == ''){
				...
				exit;
				 	...
				 	$qry_already_event= "SELECT ".$tblprefix."offers_not_included_in_price.ser_id 
					FROM
					".$tblprefix."offers_not_included_in_price where free_service='".$event_category_name."' ";
			 	 ...
			 	 $sql_category= "INSERT INTO ".$tblprefix."offers_not_included_in_price  
														SET
...
//---------Edit Category---------
	if($_POST['mode']=='update' && $_POST['act']=='update_offr_nt_inc_price' && $_POST['request_page']=='ofrs_not_inc_in_price_mange'){
	    $event_category_name = addslashes(trim($_POST['event_category_name']));
		//$pm_id = $_POST['pm_id'];
...
//$_SESSION['id']=$id;
		 $_SESSION['event_category_name']= $event_category_name; 
			...
			if($event_category_name == ''){
				$errmsg = base64_encode('Please Enter Optional Extra');
				header("Location: admin.php?act=update_offr_inc_price&errmsg=$errmsg&id=".base64_encode($_POST['id']));
				exit;
			}else{
			 	 $sql_category= "UPDATE ".$tblprefix."offers_not_included_in_price 
														SET
														free_service = '".$event_category_name."' 
														WHERE
														ser_id=".$id;
...
if($_GET['mode']=='del_xtra' && $_GET['act']=='offers_not_included_in_price' && $_GET['request_page']=='ofrs_not_inc_in_price_mange'){
		$id=base64_decode($_GET['id']);
		$del_qry = " DELETE FROM ".$tblprefix."offers_not_included_in_price WHERE ser_id = ".$id; 
