<?php	
if($_POST['mode']=='add' && $_POST['act']=='manage_yatch_commission' && $_POST['request_page']=='yatch_commision_management'){
$_SESSION["addpcommision"] = $_POST;
$post=$_POST;
$error='';
$first_name = $_POST['first_name'];
$property_name = $_POST['agency_id'];
...
$qry_comm = "SELECT * FROM ".$tblprefix."yatchdef_commission  LIMIT 1 ";
...
if($error!=''){
  ...
  $c_from_date=date("Y-m-d",strtotime($post['from_date']));
  $c_to_date=date("Y-m-d",strtotime($post['to_date']));

  
   $check_duplicate = "SELECT id FROM ".$tblprefix."yatch_commission WHERE pm_id='".$first_name."' AND agency_id='".$property_name."' AND ((from_date>='".$c_from_date."' AND to_date<='".$c_to_date."') OR (to_date >= '".$c_from_date."'
AND to_date <= '".$c_to_date."'))";
				...
				<?php	
					$error = base64_encode("Commission for this property has already been existing");
                  	header("Location: admin.php?errmsg=$error&act=".$post['act']);
                  	exit;
                  }
                  else {
				  $update_img_query = "INSERT ".$tblprefix."yatch_commission 
									                                SET
																	pm_id = '".$first_name."',
																	agency_id = '".$property_name."',
...
if($_POST['mode']=='update' && $_POST['act']=='edit_yatch_commision' && $_POST['request_page']=='yatch_commision_management'){
$post=$_POST;

$error='';
$first_name = addslashes(trim($_POST['first_name']));
$property_name = addslashes(trim($_POST['agency_id']));
...
$default_commission=$rs_comm->fields['default_commission_rate'];
	...
	$check_duplicate = "SELECT id FROM ".$tblprefix."yatch_commission WHERE pm_id=".$first_name." AND agency_id=".$property_name." 
	...
	//echo $check_duplicate;
		          ...
		          exit;     	
						 ...
						 $update_img_query = "UPDATE ".$tblprefix."yatch_commission 
							SET
							pm_id = ".$first_name.",
							agency_id = ".$property_name.",
							from_date ='".date("Y-m-d",strtotime($post['from_date']))."', 
...
if($_GET['mode']=='updatestatus' && isset($_GET['status'])){
		...
		if($oldstatus == '0'){
			...
			$update_qry = "UPDATE ".$tblprefix."yatch_commission SET 
...
if($_GET['mode']=='delete' && $_GET['act']=='manage_yatch_commission' && $_GET['request_page']=='yatch_commision_management')
{
$id=base64_decode($_GET['id']); 
	
	
	$sel_qry = "SELECT commission FROM ".$tblprefix."yatch_commission WHERE id =".$id;
		$rs_select = $db->Execute($sel_qry);
		$del_qry = " DELETE FROM ".$tblprefix."yatch_commission WHERE id =".$id;
