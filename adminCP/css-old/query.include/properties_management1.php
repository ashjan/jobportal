...
######################
//---------Add Category---------
	if($_POST['mode']=='add' && $_POST['act']=='manage_properties1' && $_POST['request_page']=='properties_management1'){
		...
		if($property_name == ''){
				...
				header("Location: admin.php?act=manage_properties1&errmsg=$errmsg");
				exit;
			}
...
//---------Edit Category---------
	...
	if($_POST['mode']=='update' && $_POST['act']=='manage_properties1' && $_POST['request_page']=='properties_management1')
	{
		...
		if($property_name == ''){
				...
				header("Location: admin.php?act=editproperties1&id=$encryptedid&errmsg=$errmsg");
				exit;
			}
		...
		if($street == ''){
				...
				header("Location: admin.php?act=editproperties1&errmsg=$errmsg&id=$encryptedid");
				exit;
			}
		...
		if($no_property_rooms == ''){
			...
			if($error!=''){
						$msg.=base64_encode($error);
						header("Location: admin.php?okmsg=$msg&act=editproperties1");
			}else{
					...
					if(!empty($_FILES['property_thumbnail']['name']))
					{
						...
						if($type!="jpg" && $type!="jpeg" && $type!="gif" && $type!="png" && $type!="PNG")
						{
							//$okmsg = base64_encode("image must be image format!");
							$okmsg = base64_encode("Slika mora biti format slike!");
							header("Location: admin.php?okmsg=$okmsg&act=editproperties1");
							exit();
						}else{
								$imagename = $image_name_rand.".".$type;
								$filename = MYSURL."graphics/thumbnail_upload/".$imagename;
...
if($_GET['mode']=='delete' && $_GET['act']=='manage_properties1' && $_GET['request_page']=='properties_management1'){
		...
		$okmsg = base64_encode("Nekretnina je uspješno izbrisana!");
					header("Location: admin.php?okmsg=$okmsg&act=manage_properties1");
					exit;	  
 }
		
//---------Delete THe Category and its language contents---------
if($_GET['mode']=='change_properystatus' && $_GET['act']=='manage_properties1' && $_GET['request_page']=='properties_management1'){
...
if($_GET['mode']=='topoffer_status' && $_GET['act']=='manage_properties1' && $_GET['request_page']=='properties_management1'){
		$id=base64_decode($_GET['id']);
...
//---------Service Provider Status-------- 
if($_GET['mode']=='goldoffer_status' && $_GET['act']=='manage_properties1' && $_GET['request_page']=='properties_management1'){
