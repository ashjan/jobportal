...
?>
			<tr <?php if($i%2==0){ ?> bgcolor="#E7DAE7"  <?php }else{ echo 'bgcolor="#FFFFFF"'; }?>>
                        ...
                        <td valign="top"><?php echo $rs->fields['yacht_booking_number']?></td>						
                        <td valign="top"><?php echo ucwords($rs->fields['customer_name'])." ".ucwords($rs->fields['customer_last_name']); ?></td>
                        <td valign="top"><?php echo ucwords($rs->fields['customer_name'])." ".ucwords($rs->fields['customer_last_name']); ?></td>											
                        <td valign="top"><?php echo date("F j Y",strtotime($rs->fields['pickup'])); ?></td>											
                        <td valign="top"><?php echo date("F j Y",strtotime($rs->fields['dropoff'])); ?></td>						
                        <td valign="top"><?php 
						$date1 = strtotime($rs->fields['pickup']);
						$date2 = strtotime($rs->fields['dropoff']);
						// Calculate the differences, they should be 43 & 11353
						echo count_days( $date1, $date2 );
						 ?></td>						
                        <td valign="top"><?php echo number_format($rs->fields['commission'],2) . "%";
						        $sum_commission[]=$rs->fields['commission'];
						 ?></td>											
                        <td valign="top"><?php //echo $rs->fields['commission']; ?></td>	<!--here i will past the Result Code-->				
                        <td valign="top"><?php echo number_format($rs->fields['amount'],2);
						$rent[]= $rs->fields['amount'];
						 ?>&euro;</td>			
                        <td valign="top"><?php
						  $comm=(float)$rs->fields['commission']/100;
						  $comm_amount=(float) ($comm*$rs->fields['amount']);
						  
						  $final_amount=$comm_amount + $rs->fields['amount'];
						...
						<!--<td valign="top">
							<a href="admin.php?act=editregion&amp;id=<?php //echo base64_encode($rs->fields['id']);?>"><img src="<?php //MYSURL?>graphics/edit.gif" border="0" title="Edit" /></a>&nbsp;&nbsp;
							<a href="admin.php?act=manage_region&amp;mode=del_region&amp;id=<?php //echo base64_encode($rs->fields['id']); ?>&amp;request_page=region_management" onClick="return confirm('Are you sure you want to Delete?')"><img src="<?php //MYSURL?>graphics/delete.gif" title="Delete" border="0" /></a>
                       </td>-->
					</tr>
