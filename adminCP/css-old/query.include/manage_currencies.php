...
if($error!=''){
			...
			header("Location: admin.php?okmsg=$msg&act=".$post['act']);
				...
				FROM
				".$tblprefix."currencies WHERE countryname='".$country."' AND curr_name='".$post['curr_name']."' AND curr_isocode='".$post['curr_isocode']."' ";    
				...
				$errmsg = base64_encode('Ova valuta već postoji.');
				...
				exit;
					 	...
					 	$update_cur_query = "INSERT ".$tblprefix."currencies SET
														...
														curr_isocode= '".strtoupper($post['curr_isocode'])."',
														currency_status = 0
														";  
...
if($_GET['mode']=='delete' && $_GET['act']=='manage_currencies' && $_GET['request_page']=='manage_currencies'){
	$id=base64_decode($_GET['id']);
		...
		$okmsg = base64_encode("Valuta uspješno izbrisana. !");
