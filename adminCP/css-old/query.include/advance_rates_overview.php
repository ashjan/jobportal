...
if($_POST['mode']=='overview_rates' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
...
if($error!=''){
	...
	if($_POST['room_type']==0){
			header("Location: admin.php?okmsg=$msg&act=get_advance_rates_overview2&id=$room_type&pm_id=$pm_id&stdt=$start_date&endt=$end_date"."&pr_id=".$property_id);
		}else {	
header("Location: admin.php?okmsg=$msg&act=get_advance_rates_overview&id=$room_type&pm_id=$pm_id&stdt=$start_date&endt=$end_date"."&pr_id=".$property_id);
		exit;
	}
}
}
...
/*if($_POST['mode']=='update_room' && $_POST['act']=='get_advance_rates_overview' && $_POST['request_page']=='advance_rates_overview'){
	...
	$qry_select = "SELECT * FROM ".$tblprefix."advance_date WHERE advance_date='".$date_con."' 
...
if($count_select > 0){
	...
	$update_query = "UPDATE ".$tblprefix."advance_date
	                 SET
	                 no_of_rooms='".$rooms."'
	...
	$insert_query = "INSERT ".$tblprefix."advance_date
	                 SET
	                 no_of_rooms='".$rooms."',
	                 advance_date='".$date_con."',
...
header("Location: admin.php?okmsg=$msg&act=get_advance_rates_overview&id=".base64_encode($propertyroomid)."&pm_id=".base64_encode($pm_id)."&stdt=".$_POST['start_date']."&endt=".$_POST['end_date']."&pr_id=".base64_encode($property_id));
exit();
}
}
if($_POST['mode']=='update_rate' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
	$error = '';
	$post = $_POST;
	...
	if($error!=''){
		...
		$insert_tariff_cal = "UPDATE ".$tblprefix."standard_rates
										SET
									    advance_rate_price =".$post['change_rate'].",
										advance_start_date  = '".date("m/d/Y",strtotime($start_date))."',
										advance_end_date = '".date("m/d/Y",strtotime($end_date))."' WHERE id=".$post['change_rates_id'];
...
if($_POST['mode']=='update_rate' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
	...
	if($error!=''){
		...
		for($i=0;$i<count($ranges);$i++){
			   $query_update ="UPDATE ".$tblprefix."changed_advance_rates
	                 SET
	                 advance_rate_price=".$post['change_rate']."
	                 WHERE property_id=".$property_id."
	                 AND advance_date='".$ranges[$i]."'
...
if($_POST['mode']=='update_rate1' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
	$error = '';
	$post = $_POST;
	...
	if($error!=''){
		...
		header("Location: admin.php?okmsg=$msg&act=get_advance_rates_overview2&id=".base64_encode($id)."&pm_id=".base64_encode($pm_id)."&stdt=".$_POST['start_date']."&endt=".$_POST['end_date']."&pr_id=".base64_encode($property_id));
		exit();
	}else{
...
if($_POST['mode']=='update_room' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
	$error = '';
	$post = $_POST;
	...
	if($error!=''){
		...
		for($i=0;$i<count($ranges);$i++){
			   $query_update ="UPDATE ".$tblprefix."changed_advance_rates
	                 SET
	                 adv_rooms_for_sale=".$no_of_rooms."
	                 WHERE property_id=".$property_id."
	                 AND advance_date='".$ranges[$i]."'
	                 AND standard_rate_id=".$rate_id."
	                 AND room_type_id=".$propertyroomid." 	                 
...
if($_POST['mode']=='update_room1' && $_POST['act']=='mng_advancerates_ovrview' && $_POST['request_page']=='advance_rates_overview'){
	$error = '';
	$post = $_POST;
