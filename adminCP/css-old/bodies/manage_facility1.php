...
if($_SESSION[SESSNAME]['pm_moduleid']==2){
 ...
 $qry_faq = "SELECT * 
            FROM ".$tblprefix."property_facilities 
			LEFT JOIN  ".$tblprefix."properties ON ".$tblprefix."properties.id=".$tblprefix."property_facilities.property_id 
            $module_pm_where " ;
...
//Dropdown for parent
$category_qry1 = "SELECT ".$tblprefix."property_manager.*,
   				   	  ".$tblprefix."properties.property_name ,
				      ".$tblprefix."properties.pm_type ,
				      ".$tblprefix."properties.property_category 
						 FROM ".$tblprefix."property_manager 
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="txt">
		  ...
		  <?php if($_SESSION[SESSNAME]['pm_moduleid']==2){?>
			...
			<div id="property_id">
					<select name="property_id"class="fields" >
				 	<option value="0">Izaberite objekatName</option>
					<?php  
 ...
 <option value="<?php echo $rs_property->fields['id']; ?>"><?php echo $rs_property->fields['property_name']; ?></option>
			...
			<select name="first_name" class="fields"   id="first_name" onchange="get_prop_name('property_ids', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name18_18.php"?>')">
...
<!--  <tr>
				<td class="txt2">Izaberite vlasnika objekta:</td>
					<td>
					<select name="pm_id" class="fields" id="pm_id" onchange="get_prop_name8('property_ids', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name8.php"?>')">
				 	<option value="0"> Izaberite vlasnika objekta</option>
					<?php
					$rs_category1->MoveFirst();
					while(!$rs_category1->EOF){
					?>
		  			<option value="<?php echo $rs_category1->fields['id'];?>"><?php echo $rs_category1->fields['first_name']."  ".$rs_category1->fields['last_name'];  ?></option>
					<?php
   ...
   <select name="fac_cat_id" id="fac_cat_id" class="fields1" 
	 onchange="get_fac_name('facility_name', this.value, '<?php echo MYSURL."ajaxresponse/get_fac_name.php"?>')">
          ...
          <input type="hidden" name="request_page" value="facility_management1" />
	...
	<tr>
				<td class="txt2">Select Property :<br/>[Izaberte objekat]</td>
					...
					if($_SESSION[SESSNAME]['pm_moduleid']!=2){
					?>
					<select name="pm_id" class="fields" id="pm_id" onchange="get_prop_name11('property_id', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name_offline.php"?>')">
				 	<option value="0"> Izaberite vlasnika objekta</option>
					<?php
					if(isset($_GET['pm_id'])){
					...
					if(isset($_GET['pr_id'])){
						...
						//Dropdown for parent
						if(isset($_GET['pr_id']) and isset($_GET['pm_id']) and !empty($_GET['pm_id']) and !empty($_GET['pr_id'])){
					...
					<?php if(isset($_GET['pr_id'])){
						$pr_id = base64_decode($_GET['pr_id']);
						}else{
						$pr_id = 0;
						} ?>
					<select name="property_id"class="fields" onchange="get_facilities('properties_facilities', this.value, '<?php echo MYSURL."ajaxresponse/get_failities_offline.php"?>')" >
				 	<option value="0">Izaberite objekat</option>
					<?php  
					$rs_property->MoveFirst();
					while(!$rs_property->EOF){ ?>
 <option value="<?php echo $rs_property->fields['id']; ?>"
 <?php if($pr_id==$rs_property->fields['id']){echo 'selected="selected"';}?>
 ...
 ><?php echo $rs_property->fields['property_name']; ?></option>
	...
	?>
    <div id="properties_facilities" style="display:<?php echo $style?>;">
	</div>
</td>
</tr>
</table>
           <?php
			...
			if(isset($_GET['pr_id']))
			{
			  $prid = base64_decode($_GET['pr_id']);
			}else {
			   $prid = $_SESSION[SESSNAME]['pm_id'];
			}
			?>
            <script language="javascript">
	        get_facilities_offline('properties_facilities', <?php echo $prid;?>, '<?php echo MYSURL."ajaxresponse/get_failities_offline.php"?>');
			</script>
            <?php }else { ?>
