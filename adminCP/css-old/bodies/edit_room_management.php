...
$totalPm = $count_pm;
	if($_SESSION[SESSNAME]['pm_moduleid']==2){
		$module_pm_where = " WHERE pm_id=".$_SESSION[SESSNAME]['pm_id']."
							 ...
							 AND ".$tblprefix."properties.id=".$property_id;
	}else{
		$module_pm_where = " WHERE ".$tblprefix."properties.pm_id=".$pm_id."
		                     AND ".$tblprefix."properties.pm_type=1 
							 AND ".$tblprefix."properties.property_category=24 
							 AND ".$tblprefix."properties.id=".$property_id;
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="txt">
   ...
   <?php if($_SESSION[SESSNAME]['pm_moduleid']==2){?>
      ...
      <tr>
			...
			<td>
			<select name="pm_id" class="fields"   id="pm_id" onchange="get_edit_room_prop_name('property_id', this.value, '<?php echo MYSURL."ajaxresponse/get_edit_room_prop_name.php"?>')">
  ...
  <tr><td>Room Type &nbsp;[Vrsta sobe] <br/><br/> Room Type (Rus) <br/><br/> Room Type (Mon)</td>
		   <td>
		...
		<a href="javascript:" style="margin:5px; width:112px; float:none; text-align:center;" name="submit" id="submit" class="button" onclick="edit_room_manag()">Update Room</a>
			...
			<input type="hidden" name="act"           value="edit_room_management"   />
			<input type="hidden" name="act2"          value="edit_room_management"   />
