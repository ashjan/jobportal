...
$qry_limit = "SELECT mv.*,pm.id as pid,pm.first_name,pr.id as prid,pr.property_name  FROM `".$tblprefix."mediaivideos` as mv INNER JOIN ".$tblprefix."properties as pr ON pr.id=mv.property_id INNER JOIN ".$tblprefix."property_manager as pm ON pm.id=mv.pm_id where mv.id=".$id." AND pr.pm_type=0";
...
if($_SESSION[SESSNAME]['pm_moduleid']==2){
	$qry_property = "SELECT
					...
					FROM
					".$tblprefix."properties WHERE pm_id=".$_SESSION[SESSNAME]['pm_id']." AND pm_type=0"; 
}else {
$qry_property = "SELECT
                    ".$tblprefix."properties.id,
					".$tblprefix."properties.property_name
					FROM
					".$tblprefix."properties WHERE pm_type=0"; 
}
...
$qry_property = "SELECT
					...
					".$tblprefix."properties.property_name
					FROM
					".$tblprefix."properties WHERE pm_id=".$rs_limit->fields['pm_id']." AND pm_type=0"; 
}
...
if($_SESSION[SESSNAME]['pm_moduleid']==2){
	$qry_rooms = "SELECT
					...
					FROM
					".$tblprefix."rooms WHERE pm_id=".$_SESSION[SESSNAME]['pm_id']." AND property_id=".$rs_limit->fields['prid']; 
}else {
$qry_rooms = "SELECT
                    ".$tblprefix."rooms.id,
					".$tblprefix."rooms.room_type
					FROM
					".$tblprefix."rooms WHERE pm_id=".$rs_limit->fields['pm_id']." AND property_id=".$rs_limit->fields['prid']; 
}

$rs_rooms = $db->Execute($qry_rooms);
$totalcountrooms =  $rs_rooms->RecordCount();
...
<option <?php echo $sel; ?> value="<?php echo $rs_property->fields['id'];?>"><?php echo $rs_property->fields['property_name'];  ?></option>
	...
	<td class="txt1"><div class="image_div_title_text">
	 <div id="room_id">
			...
			<?php while (!$rs_rooms->EOF){?>
			  <option value="<?php echo $rs_rooms->fields['id']?>"
			  <?php if($rs_rooms->fields['id']==$rs_limit->fields['room_id']){?>
			  selected="selected"<?php }?>
			  ><?php echo $rs_rooms->fields['room_type']?></option>
              <?php 
			     $rs_rooms->MoveNext();
			}?>
              <?php if($rs_limit->fields['room_id']==0){?>
              <option value="0" selected="selected">All Rooms</optgroup>
              
              <?php }else {?>
              <option value="0">All Rooms</optgroup>
              <?php }?>
			</select>
     </div>
      </div>
...
<tr>
		...
		<input type="hidden" name="act" value="mediaivideos1">
		<input type="hidden" name="act2" value="editmediavideos1">
