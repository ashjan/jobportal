...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="txt">
   ...
   <!-- <form name="managecontentfrm" action="admin.php" method="post" enctype="multipart/form-data">-->
		  ...
		  <tr>
             <td>Property Manager Name</td>
              <td>
			    <select name="first_name" id="first_name" class="fields" onchange="get_prop_name('property_name', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_vat_nam.php"?>')">
 ...
 <option value="<?php echo $rs_property->fields['id']; ?>"><?php echo $rs_property->fields['property_name']; ?></option>
			...
			<tr>
             ...
             <td>
		   <input type="text" id="service_charges_type" class="fields" name="service_charges_type" value="0" />
		   </td> 		  
		   </tr>
			<tr>
...
<tr>
					...
					<select name="pm_id" class="fields" id="pm_id" onchange="get_vat_and_tax('property_id1', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_vat_and_tax.php"?>')">
				 	<option value="0">Izaberite vlasnika objekta</option>
					<?php
					if(isset($_GET['pm_id'])){
					...
					if(isset($_GET['pr_id'])){
						...
						if(isset($_GET['pr_id']) and isset($_GET['pm_id']) and !empty($_GET['pr_id']) and !empty($_GET['pm_id'])){
                    	 $property_qry = "SELECT id,property_name,property_category FROM ".$tblprefix."properties WHERE pm_id=".base64_decode($_GET['pm_id']). ' 
						 				  AND pm_type=1
					...
					<select name="property_id"class="fields" 
					onchange= "get_rates_vat_tax('get_vat_and_tax', this.value,<?php echo $_SESSION[SESSNAME]['pm_id']; ?>, '<?php echo MYSURL."ajaxresponse/get_vat_and_tax.php" ;?>')">
				 	<option value="0">Izaberite objekat</option>
					<?php  
					$rs_property->MoveFirst();
					while(!$rs_property->EOF){ ?>
