...
if($propertyroomid ==0){
	...
	if($_SESSION[SESSNAME]['module_id']==2){
		...
		$where.=" ( (".$tblprefix."changed_advance_rates.advance_date>= ".$tblprefix."standard_rates.advance_start_date) AND (".$tblprefix."changed_advance_rates.advance_date < ".$tblprefix."standard_rates.advance_end_date) ) AND  (".$tblprefix."changed_advance_rates.pm_id=" .$_SESSION[SESSNAME]['pm_id']." AND ".$tblprefix."standard_rates.advance_use_option=1 AND ".$tblprefix."changed_advance_rates.property_id=".base64_decode($_GET['pr_id'])." ) ";
	}else {
		
		$where.=" ( (".$tblprefix."changed_advance_rates.advance_date>= ".$tblprefix."standard_rates.advance_start_date) AND (".$tblprefix."changed_advance_rates.advance_date < ".$tblprefix."standard_rates.advance_end_date) ) AND  (".$tblprefix."changed_advance_rates.pm_id=".base64_decode($_GET['pm_id'])." AND ".$tblprefix."standard_rates.advance_use_option=1 AND ".$tblprefix."changed_advance_rates.property_id=".base64_decode($_GET['pr_id'])." ) ";
	}
}else {
	if($_SESSION[SESSNAME]['module_id']==2){
		$where.=" ( (".$tblprefix."changed_advance_rates.advance_date>= ".$tblprefix."standard_rates.advance_start_date) AND (".$tblprefix."changed_advance_rates.advance_date < ".$tblprefix."standard_rates.advance_end_date) ) AND  (".$tblprefix."changed_advance_rates.room_type_id=".$propertyroomid." AND ".$tblprefix."changed_advance_rates.pm_id=" .$_SESSION[SESSNAME]['pm_id']." AND ".$tblprefix."standard_rates.advance_use_option=1 AND ".$tblprefix."changed_advance_rates.property_id=".base64_decode($_GET['pr_id']).")";
	}else {
		$where.= " ( (".$tblprefix."changed_advance_rates.advance_date>= ".$tblprefix."standard_rates.advance_start_date) AND (".$tblprefix."changed_advance_rates.advance_date < ".$tblprefix."standard_rates.advance_end_date) ) AND  (".$tblprefix."changed_advance_rates.room_type_id=".$propertyroomid." AND ".$tblprefix."changed_advance_rates.pm_id=".base64_decode($_GET['pm_id'])." AND ".$tblprefix."standard_rates.advance_use_option=1 AND ".$tblprefix."changed_advance_rates.property_id=".base64_decode($_GET['pr_id'])." ) ";
	}
}
$qry_limit = "SELECT
					".$tblprefix."rooms.room_type,
					".$tblprefix."standard_rates.id,
					".$tblprefix."changed_advance_rates.room_type_id,
					".$tblprefix."changed_advance_rates.property_id,
					".$tblprefix."changed_advance_rates.advance_date,
					".$tblprefix."changed_advance_rates.standard_rate_id,
					".$tblprefix."changed_advance_rates.advance_rate_price,
					".$tblprefix."standard_rates.advance_use_option,
					".$tblprefix."standard_rates.single_adv_rate_price,
					".$tblprefix."changed_advance_rates.adv_rooms_for_sale
					FROM
					".$tblprefix."standard_rates
					...
					INNER JOIN ".$tblprefix."rooms ON ".$tblprefix."changed_advance_rates.room_type_id = ".$tblprefix."rooms.id 
				WHERE ".$where." ORDER BY ".$tblprefix."changed_advance_rates.advance_date ASC";
...
if($propertyroomid==0){
$qry_advance_rooms ="SELECT room_type_id,advance_date,no_of_rooms,rate_id FROM ".$tblprefix."advance_date WHERE pm_id=".$pm_id." ORDER BY id ASC";
}else {
$qry_advance_rooms ="SELECT advance_date,no_of_rooms,rate_id FROM ".$tblprefix."advance_date WHERE pm_id=".$pm_id." AND room_type_id=".$propertyroomid." ORDER BY id ASC";
}
...
if($propertyroomid==0){
	$qry_changed_rates ="SELECT room_type_id,advance_date,advance_rate_price FROM ".$tblprefix."changed_advance_rates WHERE pm_id=".$pm_id." ORDER BY id ASC";
}else {
	$qry_changed_rates ="SELECT room_type_id,advance_date,advance_rate_price FROM ".$tblprefix."changed_advance_rates WHERE pm_id=".$pm_id." AND room_type_id=".$propertyroomid." ORDER BY id ASC";
}
...
/*** get rooms change***/
function get_rooms_changed($day='',$pm_id=0,$room_id=0)
{
	$day_changed = array();
	global $db;
	global $tblprefix;
	
	$qry_rooms_changed = "SELECT advance_date FROM ".$tblprefix."advance_date WHERE pm_id=".$pm_id." AND room_type_id=".$room_id;
...
function get_changed_rate($day = '', $propertyroomid=0,$pm_id=0,$property_id=0)
{
	global $db;
	global $tblprefix;
	$qry_rates_changed = "SELECT rate FROM ".$tblprefix."changed_advance_rates WHERE pm_id=".$pm_id." AND room_id=".$propertyroomid." AND property_id=".$property_id;
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
       ...
       <form name="change_rate_ranges" id="change_rate_ranges" action="admin.php" method="POST" onsubmit="return validate_contant()" enctype="multipart/form-data">
        	...
        	<input type="hidden" name="change_standard_rate_id" id="change_standard_rate_id" value="">
        	...
        	<input type="hidden" name="request_page" value="advance_rates_overview"/>
			<input type="hidden" name="act" value="mng_advancerates_ovrview" />
 ...
 ><?php echo $rs_property->fields['property_name']; ?></option>
			...
			<tr>
             <td>Property Name<br/>[Izaberite objekat]</td>
			  ...
			  ?>
			    <select name="property_id" id="property_id" class="fields" onchange="get_room_type4('room_type', this.value, '<?php echo MYSURL."ajaxresponse/get_room_type16.php"?>','<?php echo $manager_id?>')" />
			    <option value="0">Select Property</option>
	...
	<div id="room_type">
			<select name="room_type" class="dropfields" >
			  <option value="0" <?php if(base64_decode($_GET['id'])==0){echo "selected='selected'";}?>>All Rooms</option>
			  <?php if(isset($_GET['id'])){
			  	if($_SESSION[SESSNAME]['pm_moduleid']==2){
			  		$manger_id = $_SESSION[SESSNAME]['pm_id'];
			  	}else {
...
<div  id="propertyroom" style="float:left; width:1300px;margin-top:50px;"> 
<?php			
 $day_arr = array();
 $year_arr = array();
 $standard_day = array();
...
if($totalOverviews >0){?>
				 <div  style="background-color:#0066FF; color:#FFFFFF; font-weight:bold; width:100%;float:left;">
             	 Advance Rates Overview &nbsp;[Pregled naprednih cijena cijena]
		</div>
				<?php 
				$single_rate_price = '';
				$standard_rate_price ='';
			    ...
			    $rs_limit->MoveFirst();
			    while (!$rs_limit->EOF) {
			    	$date = $rs_limit->fields['advance_date'];
			    	$single_rate_array[$date][] = $rs_limit->fields['single_adv_rate_price'];
			    	$standard_rate_array[$date][] = $rs_limit->fields['advance_rate_price'];
			    	$standard_room_array[$date][] = $rs_limit->fields['adv_rooms_for_sale'];
			    	...
			    	$new_dates[] = $rs_limit->fields['advance_date'];
			    	$standard_days[] = $rs_limit->fields['advance_date'];
			    	$rs_limit->MoveNext();
			    }
			    
			    /*echo "<pre>";
			    print_r($standard_room_array);
			    echo "</pre>";
			    exit();*/
				...
				if(!empty($original_day)){
					...
					<div class="single_div">Cijena nocenja za jednu osobu</div>
     				<div class="single_div">Cijena nocenja</div>
     				<div class="single_div">Sobe za prodaju</div>
     				<div class="single_div">Rezervisano</div>
     				<div class="single_div">Otkazano</div>
					</div>';
						...
						foreach ($day_arr as $key=>$year){
							foreach ($year as $keys =>$days){
								...
								if($current_month!=$previous_month and array_intersect($days,$standard_days)){
									$previous_month = $keys;
					...
					if($check_property_id==0){
						...
						foreach ($days as $day){
							if(in_array($day,$new_dates)){
								if($propertyroomid==0){
									/*echo count($standard_room_id[$day]);
									exit();*/
					    	...
					    	case (0):?>
					    	<div  class="value_div"><img src="graphics/red.jpg" title="<?php echo $day.",Preostale sobe=".$remaining_rooms.",Cijena za jednu osobu=".$single_rate_array[$day][0].",Osnovna cijena=".$standard_rate_array[$day][0]?>" border="0"/></div>
					    <?php 
					    break;
					    	case ($remaining_rooms>0):?>
					    	<div  class="value_div"><img src="graphics/green.jpg" title="<?php echo $day.",Preostale sobe=".$remaining_rooms.",Cijena za jednu osobu=".$single_rate_array[$day][0].",Osnovna cijena=".$standard_rate_array[$day][0]?>" border="0"/></div>
					    <?php 
					    break;
					    	default:
					    		echo "";
              ...
              <!-- div for displaying booked ends-->
				...
				<?php }//if cureent month ends here
					...
					?>
						<p class="errmsg">Podaci nisu pronadeni</p>
				</div>
<?php 
 ...
 ?>				<p class="errmsg">Podaci nisu pronadeni</p>
				...
				<!--<a onclick="return set_property_room_value2('newroomvalues','<?php echo MYSURL."ajaxresponse/set_property_room_value2.php"?>');">Change value</a>--></td>
				</tr>
				<input type="hidden" name="request_page" value="advance_rates_overview"/>
				<input type="hidden" name="act" value="get_advance_rates_overview" />
				<input type="hidden" name="mode" value="update_room" />
				<input type="hidden" name="start_date" value="<?php echo $_GET['stdt']?>" />
				<input type="hidden" name="end_date" value="<?php echo $_GET['endt']?>" />
