...
$where = '';
	...
	if(isset($_GET['stdt']) && isset($_GET['endt'])){
		...
		if($start_date!='' and $end_date!=''){
			...
			$where.=" AND  ".$tblprefix."changed_advance_rates.advance_date BETWEEN '".$new_start_date."' AND '".$new_end_date."'";
...
global $db;
global $tblprefix;
$qry_standard_rooms ="SELECT advance_date,no_of_rooms FROM ".$tblprefix."standard_date WHERE pm_id=".$pm_id." AND room_type_id=".$room_id."  AND property_id=".$property_id." ORDER BY id ASC";
...
global $tblprefix;
	...
	$qry_rates = "SELECT advance_rate_price FROM ".$tblprefix."changed_advance_rates 
	                  WHERE pm_id=".$pm_id." AND property_id=".$property_id."
	                  AND room_type_id=".$room_id." AND standard_rate_id=".$standard_rate_id.
	                 " AND advance_date='".$new_date."'";
	                 
	 $rs_rates = $db->Execute($qry_rates);
	 return $rs_rates->fields['advance_rate_price'];
...
?>	
		<div  style="background-color:#0066FF; color:#FFFFFF; font-weight:bold; width:100%;float:left;">
             	 Standard Rates Availability&nbsp;[Raspolo&#382;ivost za osnovne cijene]
		</div>
				<?php 
					while(!$rs_limit->EOF){
				...
				$original_day = DatesBetween($rs_limit->fields['advance_start_date'],$rs_limit->fields['advance_end_date']);
					}
					...
					/*$standard_start_date = date("z",strtotime($date1));
					
					$standard_end_date = date("z",strtotime($date2));
					
					
					for($l=$standard_start_date;$l<=$standard_end_date;$l++){
						
						$standard_day[] = date("m/d/Y",dayofyear2date($l));
					}*/
		...
		if(!empty($original_day)){
				...
				$inner_div='<div class="inner_div">
              	...
              	foreach($days as $day){
					   ...
					   $remaining_rooms = ($rooms_for_sale+$cancelled)-$total_booking;
					     ...
					     switch ($remaining_rooms){
					    	...
					    	case (0):?>
							 <div  class="value_div"><img src="graphics/red.jpg" title="<?php echo $day.",Preostale sobe=".$remaining_rooms.",single user rate per night=".$rs_limit->fields['single_adv_rate_price'].",Osnovna cijena=".$rs_limit->fields['advance_rate_price']?>" border="0"/></div>
					    <?php 
					    		break;
					    	case ($remaining_rooms>0):?>
					    	
					    	<div  class="value_div"><img src="graphics/green.jpg" title="<?php echo $day.",Preostale sobe=".$remaining_rooms.",Cijena za jednu osobu =".$rs_limit->fields['single_adv_rate_price'].",Osnovna cijena=".$rs_limit->fields['advance_rate_price']?>" border="0"/></div>
					    <?php 
					    		break;
              	...
              	foreach($days as $day){?>
					...
					//if($year==$arr){
					if(in_array($day,$standard_day)){
						echo '<div  class="value_div">'.$rs_limit->fields['single_adv_rate_price'].'</div>'; }
						else {
