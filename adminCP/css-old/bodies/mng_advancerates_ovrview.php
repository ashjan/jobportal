...
if(isset($_GET['standard_rate_from']) && isset($_GET['standard_rate_to'])){
		...
		WHERE sr.advance_start_date>='".$standard_rate_from."' AND sr.advance_end_date<='".$standard_rate_to."'";
...
$qry_limit = "SELECT
					tbl_rooms.room_type,
					tbl_standard_rates.id,
					tbl_standard_rates.room_type_id,
					".$tblprefix."rooms.property_id,					
					".$tblprefix."standard_rates.advance_start_date,
					".$tblprefix."standard_rates.advance_end_date,
					".$tblprefix."standard_rates.advance_rate_price,
...
AND pm_type=1 AND  property_category=24';
...
<form name="date_ranges" id="date_ranges" action="admin.php" method="POST" onsubmit="return validate_contant()" enctype="multipart/form-data">
			 ...
			 <tr>
			 <td></td>
			 <td><font color="#FF0000"><?php echo base64_decode($_REQUEST['errmsg']);?></font></td>
			 </tr>
        <?php } ?>
...
<?php	}else{ ?>
        <tr>
             <td>PM Name</td>
              <td>
			    <select name="first_name" id="first_name" class="fields" onchange="get_prop_name20('property_name', this.value, '<?php echo MYSURL."ajaxresponse/get_prop_name21.php"?>')">
			...
			<tr>
             <td>Property Name<br/>[Izaberite objekat]</td>
			  ...
			  <div id="property_name"> 
			    <select name="property_id" id="property_id" class="fields" onchange="get_room_type5('propertyrooms', this.value, '<?php echo MYSURL."ajaxresponse/get_room_type15.php"?>','<?php echo $_SESSION[SESSNAME]['pm_id']?>')">
 ...
 <option value="<?php echo $rs_property->fields['id']; ?>"><?php echo $rs_property->fields['property_name']; ?></option>
		...
		<?php }?>	 <!--Dropdown for room types-->
		<?php if($_SESSION[SESSNAME]['pm_moduleid']==2){?>
		<tr>
		<td>Room type<br/>[Izaberite sobu]</td>
		<td>
		<div id="propertyrooms">
        ...
        <!--Dropdown for room types end-->
		 ...
		 <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="txt">
				...
				?>	
					...
					<tr>
					<td>
					 Pregled naprednih cijena cijena
					</td>
					</tr>
		</table>
					<?php
					...
					<td><?php 
					$date=$rs_limit->fields['advance_start_date'];
					$today=date($date);
...
?></td>
	...
	$date=$rs_limit->fields['advance_start_date'];
			...
			for($i=1; $i<=31; $i++){
	$sdate = $rs_limit->fields['advance_start_date'];
	$edate = $rs_limit->fields['advance_end_date'];
	...
	for($i=1; $i<=31; $i++){
	if(in_array($i,$values)){
	 echo'<td class=\'txtnew\'>'.$rs_limit->fields['advance_rate_price'].'</td>' ;
	}else {
...
<?php 
for($i=1; $i<=31; $i++){
$rate = $rs_limit->fields['advance_rate_price'];
...
<!--<div id="propertyroom"  style="display:block; position:absolute;display: block;float: left; position: absolute; width: -moz-max-content; width: -webkit-max-content; width: -o-max-content;">-->
