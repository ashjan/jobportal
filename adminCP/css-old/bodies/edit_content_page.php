...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="txt">
 	<tr>
  		<td id="heading">Content Management for ( <?php echo $menuname;?> )[Upravljanje sadr&#382;ajima]</td>
 	</tr>
	<tr>
	</tr>
	<tr>
...
<?php 
				...
				if($mode == "update"){
					...
					AND fld_type='content_type,".$rs_content->fields['content_slug']."'"
					;
...
<tr>
			...
			<select name="pm_id" class="fields"   id="pm_id" >
				...
				if($_SESSION['add_content_page']['pm_id']== $rs_pm->fields['id'])
				{echo 'selected="selected"';}elseif($rs_content->fields['pm_id']==$rs_pm->fields['id'])
				{
	...
	<option value="pub_pro_page" <?php if($_SESSION['add_content_page']['content_type']=='pub_pro_page')
	{
		echo 'selected="selected"';
	}
	elseif($rs_content->fields['content_slug']=='pub_pro_page'){echo 'selected="selected"';}
	
	?>>Public Property Page</option>
	<option value="sitegdt"<?php if($_SESSION['add_content_page']['content_type']=='sitegdt'){echo 'selected="selected"';}
     elseif ($rs_content->fields['content_slug']=='sitegdt'){echo 'selected="selected"';}
	
	?>>Site GDT</option>
	<option value="newsforagncy"<?php if($_SESSION['add_content_page']['content_type']=='newsforagncy')
	{echo 'selected="selected"';}
    elseif ($rs_content->fields['content_slug']=='newsforagncy'){echo 'selected="selected"';}
	?>>News For Agency</option>
	</select>
	</td>
	</tr>
...
<?php           if($totallanguages>0){ 
 ...
 // Get the currently selected translated text if exist in language content table 
			...
			if(isset($_SESSION['add_content_page']['sitegdt_'.$rs_language->fields['id']]))
			{
				$value = $_SESSION['add_content_page']['sitegdt_'.$rs_language->fields['id']];
			}else {
			if($totallang_flds>0){
		    $value=$rs_lang_text->fields['translated_text'];
			}else{
...
?>				</td>
				...
				<td>&nbsp;</td>
				<td>
			    <input type="submit" class="button" name="contentSbt" id="contentSbt" value="<?php if($page=='send'){echo 'Update Contents &nbsp;[A&#382;uriraj sadr&#382;aj]';}else {echo 'Update Contents &nbsp;[A&#382;uriraj sadr&#382;aj]'; }?> " />						
