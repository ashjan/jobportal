...
$qry_limit = "SELECT mv.*,pm.id as pid,pm.first_name,pr.id as prid,pr.property_name  FROM `".$tblprefix."mediaivideos` as mv INNER JOIN ".$tblprefix."properties as pr ON pr.id=mv.property_id INNER JOIN tbl_property_manager as pm ON pm.id=mv.pm_id where mv.id=".$id;
