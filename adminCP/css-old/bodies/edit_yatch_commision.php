...
$rs_property_manag = $db->Execute($qry_property_manag);
	    ...
	    $qry_content = "SELECT pmc.*,pm.id as pm_idd ,pm.first_name,pt.agn_name FROM `".$tblprefix."yatch_commission` as pmc INNER JOIN tbl_property_manager as pm ON pm.id=pmc.pm_id INNER JOIN tbl_yatchagency as pt ON pt.agn_id=pmc.pt_id WHERE pmc.id=".$id.$module_pm_where;
