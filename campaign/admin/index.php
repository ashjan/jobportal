...
//echo $ref['host'].'</br>'.$_SERVER['HTTP_HOST']; exit;
//echo $GLOBALS["statslog"].'</br>';
//echo $GLOBALS["pagestats"]["number_of_queries"]."\t$elapsed\t".$_SERVER['REQUEST_URI']."\t".$GLOBALS['installation_name'];exit;
...
if ($GLOBALS["commandline"]) {
  ...
  if (CHECK_REFERRER && isset($_SERVER['HTTP_REFERER'])) {
    ...
    $ref = parse_url($_SERVER['HTTP_REFERER']);
    $ref['host'] = $_SERVER['SERVER_PORT'] != '80' ? $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"] : $_SERVER['SERVER_NAME'];
